...
static int vc_dnfo_show_info(struct seq_file *m, void *v)
{


	seq_printf(m, "Display Information for User Application:\n\n");
	seq_printf(m, "     \'width\'       : %d (pix)\n",
		     vc_dnfo_info.width);
	seq_printf(m, "     \'height\'      : %d (pix)\n",
		     vc_dnfo_info.height);
	seq_printf(m, "     \'scale\'       : %s\n",
		     vc_dnfo_info.scale ? "yes" : "no");
	if (vc_dnfo_info.scale) {
		seq_printf(m, "     \'swidth\'      : %d (pix)\n",
			     vc_dnfo_info.swidth);
		seq_printf(m, "     \'sheight\'     : %d (pix)\n",
			     vc_dnfo_info.sheight);
	}
	seq_printf(m, "     \'bpp\'         : %d\n", vc_dnfo_info.bpp);
	seq_printf(m, "     \'layer\'       : %d (VC layer)\n",
		     vc_dnfo_info.layer);
	seq_printf(m,
...
static int vc_dnfo_proc_write(struct file *filp,const char *buffer,size_t count,loff_t *data)
{
	int ret;
	int i;
...
static int vc_dnfo_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, vc_dnfo_show_info, NULL);
}
static const struct file_operations vc_dnfo_proc_fops = {
	.open = vc_dnfo_proc_open,
	.read = seq_read,
	.write = vc_dnfo_proc_write,
	.llseek = seq_lseek,
...
static int __init vc_dnfo_init(void)
{
	...
	if (proc_create(DRIVER_NAME, 0660, NULL,&vc_dnfo_proc_fops) == NULL) {
		rc = -EFAULT;
